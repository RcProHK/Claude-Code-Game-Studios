...
- ADR-0010 (Accepted 2026-06-10): Mirror Moment Ceremony Ownership Split — identity vs celebration seam: #26 AvatarRenderer owns visible state + evolution-tier + snapshot/hook API (render-only); #29 MirrorMoment owns weekly ceremony (cadence + non-workout gate + reveal + screenshot prompt + celebration VFX), one-directional dep on #26. **Ratified by coupled pair: #26 v2.1 APPROVED (Pass 6, CR-17 render-only) + #29 GDD APPROVED (/design-review 2026-06-10, CR-M14 zero tier-derivation)** — all 4 validation criteria met. Key Interfaces block corrected at ratification to actual shipped contract (`avatar_evolution_milestone` + `avatar_micro_evolution` + `get_evolution_snapshot()->AvatarEvolutionSnapshot`; the Proposed draft's `evolution_tier_changed`/`capture_avatar_snapshot()` never shipped — R-2). Unblocked #26 BLOCKED gate. Closes GAP-004. [docs/architecture/adr-0010-mirror-moment-ceremony-ownership.md]
