...
- [ ] **② ceremony kind 持久化**:`LootDrop` record 加 ceremony field(FULL_CEREMONY / MICRO_ACK,string-name serialize);`get_pending_drops()` 對 reveal flow 只回 FULL_CEREMONY 件(micro_ack 件行 Rule 9 banking 路徑,唔入 reveal queue);migration:既有 record 無 field → default FULL_CEREMONY
