...
Last Updated: 2026-06-12 (+ **ADR-0012 Telemetry Data Pipeline & Privacy ✅ Accepted (contract)** — G-TEL-5 唯一硬 external gate CLEARED;#28 Story 011 flush + 012 beacon unblocked;全 18 stories Ready;dedicated 第 5 HTTP channel + token-in-body beacon + `UNIQUE(session_id,client_event_id)` dedup + retention 180d) — prior 同日: **#28 Telemetry / Analytics — 3rd Polish epic, last MVP candidate**;GDD ✅ APPROVED 同日 design-system+design-review;PR-EPIC degraded-inline REALISTIC[單一 `Telemetry` thin observer module 唔合併;baseline 16–20,~18;G-TEL-1 ADR-0008 boot-Last + #14 L593 erratum = scaffold 前提;G-TEL-2/3/4 三 CI lint 命脈;**G-TEL-5 ADR-0012 唯一硬 external gate,blocks 011 flush + 012 beacon**;純 observer 無 coupled-pair blocking,deps 全 implemented;UX=N/A 無 player UI]) — prior: 2026-06-11 (+ **#27 Onboarding Flow — 2nd Polish epic**;GDD+UX 雙 APPROVED 同日;PR-EPIC degraded-inline REALISTIC[單一 `OnboardingCoordinator` thin module 唔合併;baseline 14–18;G-OB-1 ADR-0008 tail = scaffold 前提;G-OB-2 CI lint Pillar-1 命脈;G-OB-3 ADR-0001 captured-band <100;純 downstream consumer 無 coupled-pair blocking,deps 全 implemented 可獨立推;2 新 pattern coach-mark+preview-watermark]) — prior: **#25 Combat Visual Feedback — 7th Presentation epic**;GDD+UX 雙 APPROVED 同日;PR-EPIC degraded-inline REALISTIC[單一 `CombatVisualFeedback` module 唔合併;baseline 14–18;G-CV-1 ADR-0001 兩-layer + G-CV-2 ADR-0008 = scaffold 前提;#26 anchor 非 MVP dep]) — prior: 2026-06-10 (+ **#26 Avatar Renderer + #29 Mirror Moment — coupled pair**;#26 = 第六亦最後 Presentation epic / #29 = Polish MVP-screenshot-only;兩 GDD APPROVED + ADR-0010 Accepted + 2 UX spec APPROVED 同日;PR-EPIC degraded inline REALISTIC[兩 epic 一系統一 module 唔合併 — ADR-0010 ownership seam;#26 baseline 16–20 / #29 baseline 14–18;coupled pair #26 先 implement;G-AR-3 ⇄ G-MM-3 preset 策略一齊決定;G-MM-4 cadence parity 從 #26 source 註冊]) — prior: #24 Login/Shell 2026-06-08; #22 Character Screen 2026-06-07
...
| [Avatar Renderer](avatar-renderer/EPIC.md) | Presentation | #26 | avatar-renderer.md ✅ APPROVED (2026-06-10 Pass 6 — v2.1 render-only rewrite, 8-pass saga 收線) + UX spec ✅ APPROVED (2026-06-10) | **19 stories** (18 ✅ Done + 019 ADVISORY playtest deferred) | ✅ **IMPLEMENTED 2026-06-10** (committed 8224914;full gate GREEN) — `AvatarRenderer` autoload (Character Layer 10, z_index [-10,10], 零 mutation API);render-only per ADR-0010 (CR-17/AC-30 grep 守零 ceremony code)。33 GDD ACs (30B=13U+9I+8CI / 3A playtest) + 8 CI lint + 7 UX AC。5 cross-system gates G-AR-1..5 (G-AR-1 ADR-0008 = scaffold 前提;G-AR-2 #12 depth client-LUT default;G-AR-3 #5 preset ⇄ #29 G-MM-3;G-AR-5 asset scope 36 sheet+12 hero still)。Governing ADR-0010 + 0001 (HIGH) + 0003/0006/0008/0011/0007/0009。★ Pillar 5 PRIMARY render substrate(身體就係收據)|
| [Combat Visual Feedback](combat-visual-feedback/EPIC.md) | Presentation | #25 | combat-visual-feedback.md ✅ APPROVED (2026-06-11 /design-review degraded-inline + grep-verify, NEEDS REVISION→revise→APPROVED 同 session, 0 phantom) + UX spec ✅ APPROVED (2026-06-11) | **17 stories** (5 Logic, 7 Integration, 4 Config/Data, 1 Visual/Feel) | **✅✅ IMPLEMENTED (2026-06-11, 17/17 single-session full-autonomous;cvf suite green + 3 #25 CI lint + full combined gate 0 fail;3 must-not-regress guard 落地;AC-24/28 pending-honest + UX-02/03/05 art-director external gate)** — `CombatVisualFeedback` autoload (event-driven reactive, `PROCESS_MODE_ALWAYS`, tail after {#14/#6/#5/#1});2 #25-owned CanvasLayer (CombatNumberLayer follow-viewport[10-50, world-shaken] + CombatOverlayLayer 105[全屏 immune])。output-only diegetic, **零玩家 input**。20 Rules + 5 Formula + 20 EC + 17 knobs + 35 ACs (≈30B: 1I+~27U+2CI / ~5 ADVISORY+GATED) + 8 UX AC。5 cross-system gates G-CV-1..5 (G-CV-1 ADR-0001 兩-layer amendment + G-CV-2 ADR-0008 = scaffold 前提;G-CV-3 #4 cue consumer-forward;G-CV-4 P-10 sync + 新 combat-climax-flash pattern;G-CV-5 registry 17 knobs + dual-critical)。**#26 anchor grep 證偽 → 非 MVP dep**(camera-relative focal primary)。R-13 double-shake guard + FR Test #4 + R-9 critical-kill carve-out = 三 must-not-regress。Governing ADR-0001 (HIGH) + 0009/0008/0006/0007。★ Pillar 3 per-hit reaction(眼角擒獲嘅一聲悶響)|
...
| Epic | Layer | System # | GDD | Stories | Status |
|------|-------|----------|-----|---------|--------|
| [Mirror Moment System](mirror-moment/EPIC.md) | Polish (MVP screenshot-only) | #29 | mirror-moment.md ✅ APPROVED (2026-06-10 /design-review NEEDS REVISION→revise→APPROVED 同 session) + UX spec ✅ APPROVED (2026-06-10) | **16 stories** (15 ✅ Done + 016 ADVISORY playtest deferred) | ✅ **IMPLEMENTED 2026-06-10** (coupled pair — #26 implemented 先;full gate GREEN GUT 2698/2697/0 fail + 68 .gd + 10 .sh lint exit 0;#29 suite 38 tests) — `MirrorMomentCoordinator` autoload (absolute-tail after #24,terminal;**zero tier-state** CI-MM-1 守) + `MirrorMomentFormulas` (static F1/F2/F3 + resume) + `MirrorMomentConfig` (.gd+.tres) + 4 CI-MM lint;復用 CelebrationVFXLayer 110 + ModalLayer 120 (#21,has_method-guarded + fallback 自建)。26 GDD AC + 4 CI lint (CI-MM-1..4) + 8 UX AC。7 cross-system gates G-MM-1..7 全落地 (G-MM-1 boot-order allowlist sync;G-MM-3 LOOT preset B-1;G-MM-4 cadence parity registry;G-MM-7 P-12/P-13 入庫)。Governing ADR-0010 + 0001 (HIGH) + 0003/0006/0009/0008/0005。★ Pillar 5 weekly 慶典 orchestrator(停低,認返自己)|
| [Onboarding Flow](onboarding-flow/EPIC.md) | Polish (Pre-MVP) | #27 | onboarding-flow.md ✅ APPROVED (2026-06-11 /design-review NEEDS REVISION→revise-now→APPROVED 同 session, 2 BLOCKING citation-level + 2 RECOMMENDED, 0 architectural) + UX spec ✅ APPROVED (2026-06-11 /ux-review 0 BLOCKING / 5 ADVISORY) | **16 stories** (3 Logic, 6 Integration, 3 UI, 2 Config/Data, 1 Static-CI, 1 Visual/Feel ADVISORY) | **✅✅ IMPLEMENTED (2026-06-12, 16/16 single-session full-autonomous;onboarding suite 53 tests + config/ci-lint static;full combined gate 419scr/2842 pass/0 fail/3 pending-honest;G-OB-2 lint exit 0 observe-only;016 playtest ADVISORY deferred)** — `OnboardingCoordinator` autoload (tail-append after current tail #25 CombatVisualFeedback,terminal;**zero gameplay state** G-OB-2 守) + `OnboardingOverlayLayer` CanvasLayer (captured band <100 layer 63,R-2) + 內部 `src/ui/onboarding/*` helper。first-run 4-step latch (connect host #24 / 非綁定 preview / muscle=class 課 / first-drop framing);**Pillar 2 non-blocking coach-mark** (mid-set 永不出,defer gate = 純 #1 GSM {WORKOUT_ACTIVE,REST_PERIOD,LOOT_DROP} — B-1 fix) + **Pillar 1 real-drop-only** (首件裝備=真 #15 daily drop)。observe-only consumer-forward 零 upstream patch;deps {1,2,3,9,10,15,21,24} 全 implemented + grep-verified 11 contract EXACT。8 Rules + 3 Formula (may_show/auto-dismiss/boot-resume) + 5-state FSM + 17 EC + 6 knob + 24 GDD AC + 12 UX AC。3 cross-system gates G-OB-1..3 (G-OB-1 ADR-0008 tail = scaffold 前提;G-OB-2 CI lint no-gameplay-mutator = Pillar 1 命脈;G-OB-3 ADR-0001 captured-band layer)。2 新 pattern flagged (coach-mark + preview-watermark)。Governing ADR-0008 + 0001 (MEDIUM) + 0003/0006/0009。★ first-run 引路人(唔使人教,自己就明)|
| [Telemetry / Analytics](telemetry/EPIC.md) | Polish (Pre-MVP) | #28 | telemetry.md ✅ APPROVED (2026-06-12 /design-system + /design-review 同 session degraded-inline, NEEDS REVISION→revise-now→APPROVED, 1 BLOCKING phantom + 2 REC inline-fixed) | **18 stories** ✅ created (5 Logic, 7 Integration, 3 Static-CI, 2 Config/Data, 1 Visual/Feel ADVISORY;**全部 Ready** — 011 flush + 012 beacon ✅ unblocked ADR-0012 2026-06-12) | **Ready** — `Telemetry` autoload (**boots Last** per ADR-0008,pure observer **zero downstream**;`connect_for_initial_state` order-resilience) + 內部 `src/telemetry/*` helper。**Pre-MVP PIVOT/KILL gate 量度儀器**(L331 hypothesis 兩半:glance proxy[switch-latency/foreground-ratio] + drop euphoria[rarity 分布])。**Pillar 2 硬約束 100% passive**(G-TEL-2 CI 守零 gameplay emit)+ **first-party de-id 隱私**(G-TEL-3 no-PII denylist)。15 Rules + 5 Formula (switch-latency/foreground-ratio/hit-sample/lossless-aggregate/backoff) + 5-state FSM (BOOTING/ACTIVE/FLUSHING/SUSPENDED/DEGRADED) + 18 EC + 13 knob + 4 INV-T + 22 AC + CI-1/2/3。grep-verified 14/15 upstream signal EXACT;deps {1,3,9,13,14,15,platform_detect} 全 implemented。5 cross-system gates G-TEL-1..5 (G-TEL-1 ADR-0008 boot-Last + #14 L593 erratum = scaffold 前提;G-TEL-2/3/4 三 CI lint 命脈;**G-TEL-5 ADR-0012 ✅ CLEARED 2026-06-12**)。**✅ ADR-0012 Telemetry Data Pipeline & Privacy Accepted (contract)** — endpoint pair / dedicated 第 5 HTTP channel / `UNIQUE(session_id,client_event_id)` dedup / retention 180d / opt-out 兩層(empirical 留 VS-tier)。**UX = N/A(無 player UI)**。Governing ADR-0008 + 0006/0009/0003/0004/0002 + **ADR-0012 ✅(Accepted-contract)**。★ Pre-MVP go/no-go 嘅無聲量度線(量度一切,影響零嘢)|
...
| Metric | Count |
|--------|-------|
| Total epics | 29 (16 Foundation+Core + 4 Feature + 6 Presentation + 3 Polish) |
| Ready (GDD Approved) | 28 |
| Blocked (ADR Proposed — stories auto-blocked) | 1 (#2 GymSys — ADR-0002/0004 transport VS-gated)。#28 Telemetry 011/012 ✅ unblocked 2026-06-12 (ADR-0012 Accepted-contract) |
