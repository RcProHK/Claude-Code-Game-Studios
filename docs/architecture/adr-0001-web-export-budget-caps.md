...
**Amendment 2026-06-11 (#25 G-CV-1)** — additive topology revision, **two layers** added by `CombatVisualFeedback` autoload (instantiated in `_ready`, both pre-warmed — #21/#22/#23/#24 coordinator precedent): (1) `CombatNumberLayer` (floating-damage-number Label-pool host) in the **(10, 50) band** — sort order above ParticleLayer (10), below HUDLayer (50); `follow_viewport_enabled = true` so numbers track the active Camera2D and **ride the #6 world-shake** (positionally < 100 → inside the BackBufferCopy capture band → shaken, the desired "DNF unified feel"). **The exact layer integer AND the follow-viewport-vs-reparent接駁 mechanism (how a root-viewport CanvasLayer's `follow_viewport_enabled` reconciles with the GameLayer-SubViewport residence where Camera2D lives — see Viewport residence note) is Q-CV2 ratification scope** — GDD specifies the requirement (track camera, ride shake) only; pre-ratification the number pool runs **fixed-viewport degrade** (still readable, just does not follow shake). (2) `CombatOverlayLayer` (**105**, ALWAYS) — full-screen CRITICAL/OVERKILL flash overlay (`ColorRect` + analytic `canvas_item` shader, **no texture / no art asset**; latest-wins single-instance, ≤1 full-screen blend pass). **>100 = shake/saturation/BackBufferCopy-immune** (the flash must read at full contrast regardless of world desaturation; #24 ErrorBannerLayer 111 precedent for >100 immunity) — **explicitly NOT in the BackBufferCopy capture list** (the stale-enumeration phantom-citation hazard — G-CS-7/G-IU-2/G-LS-1 precedent). **<110** so the #21 loot ceremony (CelebrationVFXLayer 110) always visually covers the combat overlay (Pillar 3「loot 永遠視覺最大」, GDD R-11). The whole #25 overlay feature is **ratification-gated** (same gated class as #5/#6 FR-1/2/3); pre-ratification = EC-20 degrade (no flash; `CRITICAL_DEGRADE_PAUSE_SEC = 0.100` compensates the lost HEAVY/CRITICAL peripheral separation; number fixed-viewport). Additive, no constraint conflict, no measurement gate beyond the ratification flag — no re-ratification of the rest of the ADR needed.
...
**Amendment 2026-06-12 (#27 G-OB-3)** — additive topology revision: added `OnboardingOverlayLayer` (**63**, PAUSABLE) immediately above LoginShellLayer (62), owned by `OnboardingCoordinator` autoload (instantiated in `_ready`, pre-warmed `visible=false` — #21/#22/#23/#24 coordinator precedent). First-run single-slot coach-mark host. BackBufferCopy capture enumeration updated to **layers 0/10/15*/50/60/61/62/63** — positional capture (< 100) means 63 is captured regardless; the explicit enumeration is updated in sync (stale-enumeration phantom-citation hazard — G-CS-7/G-IU-2/G-LS-1/G-CV-1 precedent). **<100 captured band is correct (R-2)**: the coach-mark **defers (never shows) during every world-desaturating state** (`LOOT_DROP`/`WORKOUT_ACTIVE`/`REST_PERIOD` — Formula 1 WORKOUT_CRITICAL gate / AC-10), so it is never on-screen while the world desaturates → desaturation-immunity is moot, no need to crowd the immune (100,110) band (decoupled from #25 CombatOverlayLayer 105). Constraints: >62 (above #24, topmost in-world overlay), <100 (captured band). Backdrop is **opacity-only — a 2nd BackBufferCopy is FORBIDDEN** (same ruling as #21 modal-blur CUT / #24 banner-backdrop); **no pulse / no gaze-draw animation** (#24 banner / P-17 restraint); fade opacity-only (`coach_fade_sec`, reduced-motion → hard cut); `mouse_filter = IGNORE` (UX-06, never steals one-tap). PAUSABLE (#22/#23/#24 reason). Additive, no constraint conflict, no measurement gate — no re-ratification needed.
...
│
├─ CombatNumberLayer  (CanvasLayer layer=15*, follow_viewport_enabled=true, process_mode=ALWAYS)  [#25 revision 2026-06-11]
│  └─ Floating combat damage-number Label pool (owned by CombatVisualFeedback; rides #6 world-shake)
│     *layer integer + follow-viewport-vs-reparent接駁 mechanism = Q-CV2 ratification scope; ALWAYS so rise/fade ticks during #6 hit_pause (EC-15)
│
├─ HUDLayer           (CanvasLayer layer=50,  process_mode=PAUSABLE)
│  └─ HP bar, workout-status HUD, cooldown timers  (floating combat numbers moved to CombatNumberLayer above — #25)
│
...
│
├─ OnboardingOverlayLayer (CanvasLayer layer=63, process_mode=PAUSABLE)  [#27 revision 2026-06-12]
│  └─ First-run coach-mark host (owned by OnboardingCoordinator; pre-warmed visible=false; single-slot; opacity-only, NO BackBufferCopy)
│
├─ ScreenEffectsLayer (CanvasLayer layer=100, process_mode=ALWAYS)
│  ├─ BackBufferCopy  (captures layers 0/10/15*/50/60/61/62/63 into screen texture)  [#22 revision: +60; #23 revision: +61; #24 revision: +62; #25 revision: +15* CombatNumberLayer, *provisional/ratification-scope; #27 revision: +63 OnboardingOverlayLayer]
│  └─ ColorRect (full-screen, ShaderMaterial reads u_shake_offset uniform)
│
├─ CombatOverlayLayer  (CanvasLayer layer=105, process_mode=ALWAYS)   [#25 revision 2026-06-11]
│  └─ CRITICAL/OVERKILL flash (owned by CombatVisualFeedback; ColorRect + analytic shader, no texture; >100 shake/BBCopy-immune, <110 below loot ceremony; latest-wins single-instance; ratification-gated — EC-20 degrade pre-ratify)
│
...
- **>100 = outside BackBufferCopy capture**: BackBufferCopy at layer 100 captures layers 0/10/15*/50/60/61/62/63 only (#22 revision added 60; #23 revision added 61; #24 revision added 62; #25 revision added 15* CombatNumberLayer — *provisional integer, Q-CV2 ratification scope; it rides the shake precisely because it is < 100; #27 revision added 63 OnboardingOverlayLayer) — content on **105/**110/111/120 is **immune to saturation (world −60% desaturation) and shader shake**. This is the mechanism behind #21 AC-75/AC-87 (loot burst stays fully saturated while the world desaturates; art bible「爆裝特效全飽和」), #24's `ErrorBannerLayer` (111) reading truthfully through world desaturation, and #25's `CombatOverlayLayer` (105) flashing at full contrast over a desaturated world (GDD R-11).
...
#### #25 Combat Visual Feedback layers (revision 2026-06-11 — G-CV-1)

- **`CombatNumberLayer` (layer 15*, ALWAYS, follow_viewport_enabled=true)** — single owner = `CombatVisualFeedback` autoload (instantiates in `_ready`, pre-warmed; #21/#22/#23/#24 coordinator precedent). Hosts the floating combat damage-number `Label` object pool (R-19: pool `_process` self-manages rise+fade, **no per-label Tween / no runtime alloc** — AC-29). **In the (10, 50) band, < 100 → BackBufferCopy-captured → rides the #6 world-shake** (the "DNF unified feel": numbers shake with the world they annotate). **ALWAYS (not PAUSABLE)**: the pool rise/fade `_process` must keep ticking during #6 `hit_pause` (the coordinator is `PROCESS_MODE_ALWAYS`, EC-15) — contrast the #22/#23/#24 PAUSABLE overlays which must freeze during pause. **`*` Ratification-scope (Q-CV2)**: the exact layer integer AND the follow-viewport接駁 mechanism are open — a root-viewport CanvasLayer's `follow_viewport_enabled` does not natively follow the GameLayer-**SubViewport** Camera2D (line: Viewport residence note — world content + Camera2D live inside the SubViewport, so root layers are screen-space). Two candidate resolutions: **(A)** #25-owned root CanvasLayer with an explicit world→screen anchor transform per number + shake-uniform接駁 (mirrors the #21 explicit-transform discipline); **(B)** reparent number nodes into the GameLayer SubViewport (rides camera+shake natively, no own layer). GDD names **(A)** primary; ratification picks the mechanism. **Pre-ratification degrade**: fixed-viewport (numbers spawn at a camera-relative fixed focal point, readable, do **not** follow shake) — R-17/F5, AC-18. The capture-enumeration sync (adding 15* to 0/10/.../62) follows the G-CS-7/G-IU-2/G-LS-1 positional-capture phantom-citation discipline.
- **`CombatOverlayLayer` (105, ALWAYS)** — also owned by `CombatVisualFeedback`. Full-screen CRITICAL/OVERKILL/critical-kill flash (R-8/R-10/R-9 carve-out), `ColorRect` + analytic `canvas_item` shader (**no texture / no art asset** — procedural, mirrors #6 analytic-noise stance). **Latest-wins single-instance** (a new climax replaces the active flash, never stacks → **≤1 full-screen blend pass**, saves mobile fillrate; AC-23). **>100 placement is load-bearing**: the flash must read at full contrast while the world desaturates (a desaturated climax flash would defeat the "peripheral pulse" spectacle) → shake/saturation/BackBufferCopy-immune, **explicitly NOT in the capture list** (stale-enumeration phantom-citation hazard — #24 ErrorBannerLayer 111 precedent). **<110**: the #21 loot ceremony (CelebrationVFXLayer 110) is the Pillar-3「loot 永遠視覺最大」sacred moment that always covers the combat overlay (GDD R-11; v0.2 afterglow-vs-ceremony also resolves loot-wins, EC-16). **Accessibility**: overlay opacity × #6 `motion_intensity` (=0 → no flash, photosensitivity protection, AC-25); WCAG 2.3.1 three-flashes compliance is structural (single-instance latest-wins + R-15 coalescing make >3 flash/sec impossible — successive climaxes replace, never compound).
- **Ratification gate (whole #25 overlay feature)**: same gated class as #5/#6 FR-1/2/3. AC-24 is **`pending()` gated-honesty** — pre-ratification the flash-render test must `pending("ratification-gated: ADR-0001 CombatOverlayLayer amendment")`, never assert-true false-green; ratify → real assertion. The **degrade behaviour itself (AC-07b / EC-20)** is CI-testable now: no flash + `CRITICAL_DEGRADE_PAUSE_SEC = 0.100` (replaces the 0.080 CRITICAL pause so the lost flash-based HEAVY↔CRITICAL peripheral separation is carried by a 35ms pause delta instead — still < `MAX_PAUSE_SEC = 0.12`) + number fixed-viewport. **Zero 2nd BackBufferCopy** (the overlay is a single ColorRect blend over the already-composited frame — no framebuffer copy; consistent with the #21 modal-blur CUT / #24 banner-backdrop ruling).

#### #27 Onboarding Flow layer (revision 2026-06-12 — G-OB-3)

- **`OnboardingOverlayLayer` (63, PAUSABLE)** — single owner = `OnboardingCoordinator` autoload (instantiates in `_ready`, pre-warmed `visible=false`; #21/#22/#23/#24 coordinator precedent). Hosts the single-slot first-run coach-mark card. Immediately above #24 LoginShellLayer (62). **<100 captured band — load-bearing-by-irrelevance (R-2)**: unlike the >100 immune layers (#21 110 / #24 banner 111 / #25 105) whose content MUST read through world desaturation, the coach-mark is the opposite — it **defers (never shows) during every world-desaturating state** (`LOOT_DROP` ceremony −60%, plus `WORKOUT_ACTIVE`/`REST_PERIOD` — Formula 1 WORKOUT_CRITICAL gate / AC-10), so it is **never on-screen while the world desaturates**. Desaturation-immunity is therefore moot, and the captured band (<100) is correct — no need to crowd the immune (100,110) band (decoupled from #25 CombatOverlayLayer 105). **Capture enumeration sync**: capture is positional (everything < 100 captured; `src/autoload/screen_effects.gd` L370-371), so 63 is captured regardless; the explicit enumeration is updated to **0/10/15*/50/60/61/62/63** anyway (stale-enumeration phantom-citation hazard — G-CS-7/G-IU-2/G-LS-1/G-CV-1 precedent). PAUSABLE (#22/#23/#24 reason): coach-marks only show in non-critical windows, so the layer never needs to run while the tree is paused (inert during #6 `hit_pause`/ceremony freezes). Ordering: >62 (above #24, topmost in-world overlay), <100 (captured band). **Backdrop = opacity-only — a 2nd BackBufferCopy is FORBIDDEN** (same ruling as #21 modal-blur CUT / #24 banner-backdrop; the card is a small peripheral text panel, no blur). **No pulse / no gaze-drawing animation** (#24 banner / P-17 restraint — teaching is a state, not an urgency gesture); fade in/out is opacity-only via `coach_fade_sec` (reduced-motion → hard cut). `mouse_filter = IGNORE` (UX-06 — never steals the player's one-tap).
...
| combat-visual-feedback.md (#25) | G-CV-1 / Q-CV2 / R-11: two #25-owned CanvasLayers (CombatNumberLayer follow-viewport 10-50 captured; CombatOverlayLayer 105 immune) | Amendment 2026-06-11: CombatNumberLayer added to captured band (15* provisional, Q-CV2 ratification scope); CombatOverlayLayer 105 >100 immune, NOT in capture list, <110 below loot ceremony; overlay ratification-gated (EC-20 degrade, AC-24 `pending()`) |
| onboarding-flow.md (#27) | G-OB-3 / R-2: one #27-owned CanvasLayer (OnboardingOverlayLayer coach-mark host) | Amendment 2026-06-12: OnboardingOverlayLayer 63 in captured band (<100, +63 to capture enumeration); coach-mark defers during all world-desaturating states so immunity is moot (R-2); opacity-only NO 2nd BackBufferCopy; pre-warmed hidden; single-slot; no pulse |
