...
**Amendment 2026-06-12 (#27 G-OB-1)** — added the #27 OnboardingCoordinator insertion rule (tail append after CombatVisualFeedback — i.e. now last in the autoload list, before reserved #28 Telemetry). first-run choreography / coaching layer. Predecessor set `{GSM (#1, C6), GymSysBackendClient (#2), PersistenceLayer (#3), WorkoutStateTracker (#9), ExerciseClassMapping (#10), LootRevealCoordinator (#21), LoginShellCoordinator (#24), … every existing coordinator}` all boot far earlier. **NO binding constraint on any predecessor** — #27 is a pure downstream observe-only consumer: it subscribes #9 workout signals / #21 `modal_dismissed` / #24 shell **at runtime** (plain `connect`, not boot), reads the persisted onboarding latch, and `_ready()` sync-reads **NO** upstream → tail-append carries zero boot-dependency violation (the #24 G-LS-2 / #25 G-CV-2「tail slot is convention」precedent — the GDD's early "after #29" wording was stale, B-2 fix; current tail at impl time = #25 CombatVisualFeedback). **TERMINAL** — like #25/#29, #27 emits nothing another autoload consumes (it holds **ZERO** gameplay-mutating state, G-OB-2), so nothing need boot after it. Now **written into `project.godot`** by #27 story 001 (this amendment ships with the registration, like #25/#29). ADR-0001 G-OB-3 (the #27-owned `OnboardingOverlayLayer`, captured-band <110) is the separate companion amendment (#27 story 013). No re-ratification needed (additive tail; canonical map unchanged below — #27 not in the 2026-05-29 snapshot).

**Amendment 2026-06-12 (#28 G-TEL-1)** — promoted the #28 Telemetry reserved "Last" rule to an **actual entry**: tail-append after OnboardingCoordinator (#27, the impl-time current tail — i.e. now genuinely last in the autoload list). Pre-MVP PIVOT/KILL gate instrument — a pure passive observer with **ZERO downstream consumers** and **ZERO gameplay-mutating state** (G-TEL-2 CI lint). **NO binding constraint on any predecessor** — #28 subscribes #9 workout / #14 combat / #15 loot signals **at runtime** and uses `connect_for_initial_state` to back-fill #1 GSM current_state for late join, so booting Last captures every runtime-emitted signal with zero silent drop (the combat signals fire during CombatActive / boss-kill, far after every autoload `_ready()`). **TERMINAL** — the canonical "Last" slot; nothing boots after it (#28 is reserved Last precisely so it observes a fully-booted tree; should a future autoload ever need a later slot, #28 is pushed further down — terminal observer is always last). This amendment **also resolves the Q-T1 cross-doc erratum**: `design/gdd/enemy-director.md`'s "#28 must boot BEFORE #14" wording pre-dated this canonical map (GAP-002) and is corrected to "boots Last per ADR-0008; combat signals are runtime so late-boot catches all" (#28 story 001 doc edit). Now **written into `project.godot`** by #28 story 001 (this amendment ships with the registration, like #25/#27/#29). No re-ratification needed (additive terminal tail; canonical map unchanged below — #28 not in the 2026-05-29 snapshot).

**Amendment 2026-06-11 (#25 G-CV-2)** — added the #25 CombatVisualFeedback insertion rule (tail append after MirrorMomentCoordinator — i.e. now last in the autoload list, before reserved #28 Telemetry). Predecessor constraints `{EnemyDirector (#14), ScreenEffects (#6), ParticleSystemWrapper (#5), GameStateMachine (#1, C6)} ≺ CombatVisualFeedback (#25)` — #25 subscribes #14's `hit_resolved`/`enemy_killed` via `connect_for_initial_state` + #1 GSM (lifecycle Active/Suspended); at runtime routes particle via #5 `play()`, shake via #6 auto-dispatch, hit_pause via #6 direct call. **NO #29 MirrorMomentCoordinator constraint** — the tail slot after #29 is convention, not binding: #25 and #29 have zero boot-order dependency (#25 is a combat-event reactive consumer, #29 a weekly-ceremony coordinator; neither reads the other). **TERMINAL** — like #29, #25 emits nothing another autoload consumes, so nothing need boot after it. Now **written into `project.godot`** by #25 story 001 (this amendment ships with the registration, like #29). ADR-0001 G-CV-1 (the two #25-owned CanvasLayers `CombatNumberLayer`/`CombatOverlayLayer 105`) is the separate companion amendment (#25 story 002). No re-ratification needed (additive tail; canonical map unchanged below — #25 not in the 2026-05-29 snapshot).

**Amendment 2026-06-10 (#29 G-MM-1)** — added the #29 MirrorMomentCoordinator insertion rule (tail append after LoginShellCoordinator — i.e. last in the autoload list, before reserved #28 Telemetry). **One binding constraint: `AvatarRenderer (#26) ≺ MirrorMomentCoordinator (#29)`** (constraint 4b extension) — #29 subscribes #26's `avatar_evolution_milestone`/`avatar_micro_evolution` via `connect_for_initial_state` and reads `get_evolution_snapshot()`, one-directional #29→#26 per ADR-0010 (no back-edge: #26 never references #29). All other predecessors {GSM (#1, C6), PersistenceLayer (#3), ParticleSystemWrapper (#5)} boot far earlier. **TERMINAL** — #29 is the only autoload with zero downstream (no system reads its output), so nothing need boot after it; the #21/#22/#23/#24 layer coordinators carry no boot-order constraint on #29 (it reuses #21's 110/120 via runtime has_method-guarded accessors, zero boot dependency). Now **written into `project.godot`** by #29 story 001 (this amendment ships with the registration, unlike #22-#24 which deferred). No re-ratification needed (coupled pair #26+#29 ratified ADR-0010 2026-06-10).
...
| **#28 Telemetry** | Pure observer, no downstream consumers. Order-resilient (`connect_for_initial_state` back-fills initial state for late join). | **Last** — boots after all producers so it subscribes to a fully-booted tree — ✅ executed 2026-06-12 (#28 story 001, G-TEL-1; tail-append after OnboardingCoordinator #27, now genuinely last) |
...
| **#25 CombatVisualFeedback** (added 2026-06-11 G-CV-2, GDD APPROVED) | `{EnemyDirector (#14), ScreenEffects (#6), ParticleSystemWrapper (#5), GameStateMachine (#1, C6)} ≺ #25` — subscribes #14 `hit_resolved`/`enemy_killed` via `connect_for_initial_state` (boot-current reaction) + #1 GSM (lifecycle Active/Suspended, EC-08/15); at runtime routes particle via #5 `play()` (HIT_LIGHT/HIT_HEAVY), shake via #6 auto-dispatch (#25 issues **zero** direct `ScreenEffects.shake()` — R-13 guard / AC-11), hit_pause via #6 direct call (fills the #6 auto-dispatch `pause=0` gap for HIT_HEAVY/DEATH). Reactive combat-event consumer — `PROCESS_MODE_ALWAYS` (receives `hit_resolved` while #6 HitPaused). **NO #29 MirrorMomentCoordinator constraint** — the tail slot after #29 is convention, not binding: #25 (combat-event) and #29 (weekly ceremony) have zero boot-order dependency, neither reads the other (#24 G-LS-2「NO #21/#22/#23 constraint」precedent). Explicitly NOT listed (zero boot touchpoints, #25 GDD): #13 CombatResolver (pure static, no autoload) / StatSystem / AvatarRenderer (#26 anchor API does not exist — render-only per ADR-0010; #25 uses camera-relative fixed focal point, R-17/F5). Owns CombatNumberLayer (follow_viewport, sort 10-50, inside #6 world-shake uniform) + CombatOverlayLayer (105, >100 immune) — ADR-0001 G-CV-1 revision (#25 story 002). | **Tail append after MirrorMomentCoordinator** (#28 Telemetry still reserved Last) — ✅ executed 2026-06-11 (#25 story 001) |
| **#27 OnboardingCoordinator** (added 2026-06-12 G-OB-1, GDD APPROVED) | `{GameStateMachine (#1, C6), GymSysBackendClient (#2), PersistenceLayer (#3), WorkoutStateTracker (#9), ExerciseClassMapping (#10), LootRevealCoordinator (#21), LoginShellCoordinator (#24), … every existing coordinator} ≺ #27` — pure downstream observe-only consumer: subscribes #9 `workout_started_forwarded`/`dominant_class_changed`/`workout_completed_forwarded` + #21 `modal_dismissed` + #24 shell **at runtime** (plain `connect`, not boot-time), reads the persisted onboarding latch via #3; `_ready()` sync-reads **NO** upstream → tail-append carries zero boot-dependency violation. Holds **ZERO** gameplay-mutating state (Pillar 1, CI lint G-OB-2). **NO binding constraint on #25/#29 or any layer coordinator** — the tail slot is convention, not binding (#24 G-LS-2 / #25 G-CV-2 precedent; the GDD's early "after #29" wording was stale — current tail at impl = #25). Explicitly NOT listed (zero boot touchpoints, #27 GDD): StatSystem / AbilitySystem / AvatarRenderer / AudioManager / ScreenEffects / CameraController (#27 mutates no gameplay state and calls no presentation-effect API; first-run coaching is overlay-only). Owns OnboardingOverlayLayer (captured-band <110 — ADR-0001 G-OB-3, #27 story 013). | **Tail append after CombatVisualFeedback** (#28 Telemetry still reserved Last) — ✅ executed 2026-06-12 (#27 story 001) |
...
| combat-visual-feedback.md (#25) | CombatVisualFeedback tail-appends after MirrorMomentCoordinator; preds {#14, #6, #5, #1}; NO #29 constraint; terminal | Insertion rule (tail, G-CV-2); #14/#6/#5/#1 all boot earlier |
| onboarding-flow.md (#27) | OnboardingCoordinator tail-appends after CombatVisualFeedback; pure observe-only consumer (#9/#21/#24 runtime connect, no boot sync-read); NO binding pred constraint; zero gameplay state (G-OB-2); terminal | Insertion rule (tail, G-OB-1); all preds boot earlier |
