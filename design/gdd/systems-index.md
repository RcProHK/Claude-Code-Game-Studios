...
| 33 | Attention Budget & Interaction Policy (added per CD-SYSTEMS) | Core | Pre-MVP | **Designed 2026-06-04 (pending /design-review)** — 8 sections authored autonomous-inline；Pillar 2 PRIMARY enforcement via ADR-0006 C13 `IInputPolicy`。核心決定：`is_input_permitted()` = pure pull-based 兩層 derivation（live GSM current_state + WST phase，**無 cached gate** → 架構性滿足 phone-lock recovery contract #4）；WST `SET_ACTIVE` phase 壓倒 GSM state（Rule 3 = hard-contract #1）；unlock gesture exempt（Rule 6 = 解決 #20 fallback AC-EC-S5）。9 Rules + 2 Substate + 3 Formulas（is_input_permitted / is_notification_permitted / glance_within_budget）+ 14 EC + 17 AC（16 BLOCKING + 1 ADVISORY）+ 3 tuning knobs + 2 constitutional const。Registry: `is_input_permitted_api` #33+#20 active referrer + 新增 `glance_budget_ceiling_ms`=2000。Open Q-OQ1（ADR-0008 absolute pos）。NEXT: fresh-session `/design-review design/gdd/attention-budget-policy.md` → `/create-epics` → `/create-stories`。 | [attention-budget-policy.md](attention-budget-policy.md) | 1, 9 |
