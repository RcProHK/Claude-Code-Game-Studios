...
| 33 | Attention Budget & Interaction Policy (added per CD-SYSTEMS) | Core | Pre-MVP | **✅ Approved 2026-06-04 (pass 3 full 4-agent review — all BLOCKING resolved inline)** — Hybrid model（`is_input_permitted() = GSM_floor AND LOOT_DROP ceremony-lock AND WST_refinement + sentinel`）。**Pass 3 新 fix**：B1 KNOWN_GSM/WST sentinel(enum OOB→fail-OPEN 修正)；B2 WORKOUT_COMPLETE 移出 GSM gate-table column(compile-trap)；MAJOR-5 CI ban regex GSM owner-exempt(game_state_machine.gd:272,防 main RED)；CRITICAL-4 FAIL_CLOSED→const(移出 Tuning Knobs)；G1 AC-18 split 18a BLOCKING+18b deferred；CRITICAL-3/1 AC-20/AC-21 arity+has_method guard；AC-01a SET_ACTIVE cartesian；AC-15 removed→#8/#28；cross-flag #21 ceremony timing。Review log: design/gdd/reviews/attention-budget-policy-review-log.md。 | [attention-budget-policy.md](attention-budget-policy.md) | 1, 9 |
