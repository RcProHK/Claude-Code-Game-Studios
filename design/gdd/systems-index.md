...
| 20 | Gym-Mode HUD (inferred) | Presentation | MVP | **Designed 2026-06-03 (pending /design-review)** — full review-mode authoring, 7 specialists (creative-director/game-designer/ux-designer/ui-programmer/systems-designer/art-director/qa-lead). **CD-GDD-ALIGN CONCERNS (accepted)** 0 BLOCKING, 4 inline-accepted findings. Pillar 2 PRIMARY owner「餘光戰報 / 沉默見證者」. 13 CR + Info Tier table + GSM×HUD matrix + 3-state thin-view machine + banner soft-gate + bfcache reconcile + 3 formulas (exp_fill/tween/banner pulse) + 22 EC + 11 owned knobs + ~30 AC (BLOCKING logic + ADVISORY visual/feel + 4 dep-gated fallback). **Absorbs #4 audio EG-2**: banner soft-gate (CR-6/7/8) + audio-trigger consumer (CR-9/10/11). DECISIONS: HP=MAX_HP non-depleting (depleting bar deferred, no current-HP owner); skills=get_unlocked_abilities (equip→#30). 10 Open Q. **Bidirectional gaps to patch**: #2 add #20 as set_logged subscriber; #8 expose streak signal. NEXT: /design-review (fresh session) → /ux-design gym-mode-hud (MUST define AC-V-1 glance playtest protocol). | [gym-mode-hud.md](gym-mode-hud.md) | 11, 12, 4, 2, 9 |
