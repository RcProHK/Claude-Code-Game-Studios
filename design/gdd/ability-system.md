...
| **#26 Avatar Renderer** | listens | Subscribe `ability_cast` (render-only cast animation, e.g. STRIKE_TIER_3 punch anim) + `ability_unlocked` (ability_count feeds Formula 2 evolution tier) — render-only per ADR-0010 | None | v0.2 layered character system 會深化呢個 dependency；MVP 用 placeholder animation |
...
| **#26 Avatar Renderer** | VS (Not Started) | Subscribe `ability_cast` (render-only cast anim) + `ability_unlocked` (ability_count → Formula 2 tier); render-only per ADR-0010 | ⚠️ v0.2 layered character system 會深化呢個 dependency；MVP 用 placeholder animation |
