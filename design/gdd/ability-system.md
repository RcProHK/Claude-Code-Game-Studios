...
| **#18 PR Detection & Avatar Progression** | emits signal (consumed by Ability System) | Emits `pr_breakthrough(stat_id, magnitude)` signal；AbilitySystem internal handler subscribes 同 calls `_evaluate_unlock` → `unlock_ability`；#18 NOT directly call `unlock_ability` (signal subscription pattern per Rule 3 + Rule 7) | None | Pre-MVP system — VS-tier 提供 mock/provisional path；mock 需要 emit same `pr_breakthrough(stat_id, magnitude)` signal interface |
