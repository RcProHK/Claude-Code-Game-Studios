...
> **Status**: In Design — Revised (Pass 1 MAJOR REVISION NEEDED 2026-06-06 → revision pass 同日;pending fresh re-review)
> **Author**: frank + agents(原稿 degraded inline 2026-06-06;Pass 1 full /design-review [5 specialists + CD] 後本 revision 按 CD Position A–F 裁決 + 16 BLOCKING clusters 重寫 — 見 `design/gdd/reviews/pr-detection-review-log.md`)
...
- **INV-PR-2 — Magnitude log-additivity(farming 自我平衡)**:對任意 confirmed-PR 步進序列(initial → final e1RM):`ln(final/initial) ≤ Σ pr_magnitude ≤ (final−initial)/initial`。即 micro-stepping 收多倍事件數但 magnitude 總量不增(convexity 仲益大步)— stat 面 farming 結構性無利可圖。呢條(+ #11 diminishing)先係真防線;property test AC-31。

**Avatar Progression scope(ADR-0010 邊界)**:#26 AvatarRenderer **own** evolution tier 計算(由 #11 stat + #12 ability sync read derive)—— 本系統**唔重複**呢個邏輯。#18 嘅 progression 角色係**事件源**:PR → stat delta → #26 tier 自然推進;PR → ability unlock → #26 posture 變化。另加 PR milestone tracking(`pr_milestone_reached` — **#19 Zone System 已接做 unlock kind 之一**,zone-system.md L22/L43/L64;thresholds PROVISIONAL 待 Q-PR-1 校準)。
...
> - **D8(Position D 裁決)** — **Soft-confirm 防 typo fabrication**:`pr_magnitude > SUSPECT_PR_MAGNITUDE`(0.30)→ 唔即時生效,入 **PENDING_CONFIRMATION**(delta / signal / count / baseline 全 hold)。後續同 exercise set(同 session 或下一 session 內)計出 e1rm ≥ pending × CORROBORATION_RATIO → commit 全額(delta 以 clamped magnitude 計,baseline 升到 raw e1rm);到「下一個 workout 嘅 workout_completed」仍無 corroboration → discard + `pr.pending_discarded`。理由:GymSys 係手動入數,kg/lb typo 係最高發生率嘅 fabrication vector;生理上 30%+ e1RM 單跳接近不可能,而 corroboration 對真 detraining-return 玩家只係延遲一兩個 set。GymSys 改返正確數字 → 該 set 喺 server 消失 → 永不 redeliver → pending 自然 discard(catch-up replay 來自 server state — 自我修正)。
...
6. **PR 生效(順序 binding)** —
   ...
   7. emit `pr_breakthrough(stat_id, pr_magnitude)`(**emit gate**:#12 `boot_completed` 已 fire + GSM 非 Suspended — #12 EC-16 binding 義務;結構上 6.3 成功已蘊含 #11 非 suspended + boot chain 完成,gate 係 belt-and-braces assert,AC-30)+ `pr.detected` telemetry(exercise_id / magnitude / stat_id / delta)。
...
8. **Baseline store** — 單一 persist key **`pr.state`**(SerializableResource envelope,ADR-0009):`{ baselines: Dictionary[String, float], pending: Dictionary, candidates: Dictionary, candidates_workout_unix: int, lifetime_count: int, lifetime_pr_score: float }`。**理由(binding)**:#3 IPersistence **冇 key-enumeration API**(shipped surface = exact-key read/write/delete/migrate)— per-key `pr.best.*` boot load unimplementable;PersistenceLayer 係 whole-cache snapshot flush,per-key 細分零 I/O 著數。Boot 時:(a) `read("pr.state")` 還原;(b) 經 #2 polling state response 攞 server baselines(G-PR-1,ADR-0011 §D-2.4 — 隨 polling 落地,唔開獨立 endpoint,結構性保證 baseline 先於 catch-up replay);(c) per-entry validate 後 server 覆寫 local(D2;session-confirmed floor);(d) server unavailable → 有 local cache 嘅 exercise 照判定(grace),**無 local cache 嘅 exercise 行 INV-PR-1 fail-closed**(establishment-only — 封死「block baseline endpoint + 清 IndexedDB 每 session farm」攻擊面)。
9. **Milestone tracking** — `lifetime_count` 達 `PRMilestoneConfig.tres` thresholds([10, 25, 50, 100] **PROVISIONAL** — 零 gameplay 驗證,#19 v0.2 接用前 Q-PR-1 校準)→ emit `pr_milestone_reached(count)`。**Boot 載入已過 threshold 唔 re-emit**(crossing-only 語意,AC-19)。**Consumer**:#19 Zone System 已定義 `PR_MILESTONE` unlock kind(zone-system.md L22/L43/L64)— **CD directive:#19 MVP zone 唔用呢個 kind(用 streak/workout-count),PR_MILESTONE 留 v0.2**;`lifetime_pr_score`(Σ magnitude)同步 persist,係 #19 將來改用 magnitude-weighted 軸嘅 data surface(#19 design space,#18 只保證數據可取)。Config thresholds 非 ascending → `validate_milestone_config() == false` + push_error(EC-12;AC-20 — 唔用 raw assert,GUT headless 測唔到)。
10. **Boot / lifecycle** — autoload `PrDetection` @ `src/autoload/pr_detection.gd`(**ADR-0011 §D-4**;`src/feature/` 係 phantom path — 全部 17 個 shipped autoload 喺 `src/autoload/`;`check_stat_mutation_callers.gd` whitelist amend 落 epic 第一個 story)。Gate G-PR-3:ADR-0008 insertion — **full chain `GymSysBackendClient ≺ ExerciseClassMapping ≺ StatSystem ≺ WorkoutStateTracker ≺ AbilitySystem ≺ PrDetection`**(#18 排鏈尾 — 兩個 reverse-wire target 都先 boot)。`_ready` 內 **synchronous** 完成 INITIALISING → READY(load local → 接線 → READY;項目慣例 — WST/#3 同款;#2 事件全部經 async HTTP callback boot frame 後先到,結構上零 pre-READY window;server baseline sync 係 async 唔 block,行 BASELINE_SYNCING substate)。**Reverse-wiring(boot 時 #18 主動 connect 自己 signal 落兩個 consumer)**:`pr_breakthrough.connect(AbilitySystem._on_pr_breakthrough)`(G-PR-4 — handler 已 grep pin:`ability_system.gd:895`,簽名 `(stat_id: StringName, magnitude: float)` match;shipped comment L884-888 明文留咗呢個做 #18 stable entry point)+ `pr_breakthrough.connect(#9 G-PR-2 新 handler)`。方向理由:唔係「#12 訂唔到未存在嘅 #18」(Godot autoload 兩段式 boot 下,所有 autoload 喺任何 `_ready` 前已 instantiate — 經 global name connect 未-ready node 嘅 signal 合法且 deterministic)— 而係 (a) #12 shipped code 已將 `_on_pr_breakthrough` 定位做「#18 connect 嘅 stable entry point」,ownership 已定;(b) 項目紀律唔依賴未-ready instance。GSM 經 `connect_for_initial_state`(Contract 6)— #18 自己無 SUSPENDED queue 需求(#2 own delivery;#18 stateless per-event + D5 replay-safe),跟 GSM 做 telemetry 靜默 + Rule 6.7 emit gate。
...
| State | 意義 | 入 | 出 |
|-------|------|----|----|
| `INITIALISING` | `_ready` 內 synchronous:load `pr.state` → reverse-wire #12/#9 → 訂 #2 → 發 async baseline sync 請求(唔 block) | boot(Contract 4) | `_ready` 尾 → `READY`(同 frame) |
...
| **#19 Zone System** | #19 own unlock framework | — | `pr_milestone_reached(count)` + `lifetime_pr_score` data surface | **#19 已接做 unlock kind**(zone-system.md L22/L43/L64);CD directive:MVP zone 唔用,v0.2 + Q-PR-1 校準後 |
...
# Server baseline 喺 window 期間到達(validated)→ 即時採納,該 exercise window 即終止,
# 之後嘅 set 正常判定(against server baseline);已累積 candidate 棄用。
```
...
- **EC-16 (MEDIUM) — Server baseline entry invalid(0 / 負數 / 非有限 / 超 sanity)**:per-entry reject(保留 local)+ `pr.baseline_invalid`(ADR-0011 §D-2.2;AC-23)。封 ÷0 → INF → clamp 2.0 假 max PR 同負數永久壓制兩個 degenerate。
...
跟 shipped 慣例(#15/#17 pattern):internal `_telemetry_log: Array[Dictionary]` ring + `_emit_telemetry(event: StringName, data: Dictionary)` helper — test assertion surface(seam 5)+ future #28 forwarding。Events:`pr.detected` / `pr.input_invalid` / `pr.unknown_exercise` / `pr.baseline_established` / `pr.magnitude_anomaly` / `pr.baseline_conflict` / `pr.baseline_invalid` / `pr.pending_opened` / `pr.pending_corroborated` / `pr.pending_discarded` / `pr.weight_novelty_no_pr` / `pr.late_set` / `pr.replay_recheck`。
...
> GWT;Logic = unit-testable。**Test seams(8,對齊 #17 慣例)**:① #2 signal 注入(set_logged / workout_started / workout_completed)② #2 baseline async response mock(capture-and-release — AC-14/15/16/23 ordering)③ #10 mock ④ #11 mock(apply_stat_delta return 控制 + get_stat)⑤ telemetry spy(ring 讀取 — #17 seam 5 同款)⑥ persistence spy/mock ⑦ #12/#9 handler spy(signal 接收驗證)⑧ GSM mock(emit gate / 靜默)。**無 TimeProvider seam**(#18 無 clock 依賴;daily reset 係 #9-side — G-PR-2 spec)。Epsilon:golden 數值 assert ±0.001(除非另注)。

### Detection pipeline
- **AC-01 (Logic)** — **GIVEN** trusted baseline bench=70.0 **且 STR=12.0**,**WHEN** `set_logged("bench_press", 5, 65.0)`(e1rm 75.83),**THEN** `apply_stat_delta(&"str", PR_BREAKTHROUGH, δ)` called once,δ ≈ **0.500**(±0.001 — AC-12 golden 重用)、`pr_breakthrough(&"str", 0.0833 ±0.001)` emitted、baseline → 75.83、`pr.state` persist flush=true 一次、`pr.detected` telemetry emitted。
...
- **AC-07 (Logic)** — **Soft-confirm 三路徑(D8/EC-5)**:**GIVEN** baseline 70.0,**WHEN** `set_logged` 計出 raw m = 3.5(typo 級),**THEN** PENDING opened(零 delta / 零 signal / 零 count / baseline 不變)+ `pr.magnitude_anomaly` + `pr.pending_opened`;**(a) corroborate**:後續 set e1rm ≥ pending × 0.95 → commit(delta 以 **clamped 2.0** 計;baseline 升 **raw** e1rm;signal magnitude = 2.0)+ `pr.pending_corroborated`;**(b) discard**:下一 workout `workout_completed` 仍無 corroboration → pending 清空、baseline 不變、`pr.pending_discarded`;**(c) 正常 anomaly**:m = 0.25(< SUSPECT)→ 即時 confirmed,無 pending。
...
- **AC-23 (Logic)** — **Server entry validation(EC-16)**:**GIVEN** server 回 `{a: 0.0, b: -10.0, c: INF, d: 85.0}`,**THEN** a/b/c reject(local 保留)+ `pr.baseline_invalid` ×3;d 正常採納(ADR-0011 §D-2.2)。
...
- **AC-27 (Logic)** — Boot ordering:load `pr.state` **先於** subscribe #2(injection order assert);`_ready` 完結時 state == READY(synchronous,INITIALISING 唔跨 frame)。
...
- **AC-30 (Logic)** — **Emit gate(#12 EC-16)**:**GIVEN** GSM mock = Suspended,**WHEN** PR pipeline 行到 6.7,**THEN** `pr_breakthrough` 唔 emit(buffer 一格,resume 時 flush)— 配合 6.3(#11 suspended 時 reject → 通常 abort 喺上游);GSM 靜默:state 轉換時 #18 零 active 行為(telemetry only)。
...
- **PR_MILESTONE zone unlock**(#19 v0.2;Q-PR-1 校準 + 考慮 Σmagnitude 軸)。
...
| **G-PR-3** | ADR-0008 insertion:**full chain `#2 ≺ #10 ≺ StatSystem ≺ WST ≺ AbilitySystem ≺ PrDetection`**(#18 鏈尾 — 兩個 reverse-wire target 先 boot) | autoload boot | ADR-0008 focused amendment(照 #17 G-4 先例) | technical-director |
...
| **G-PR-5** | **#12 additive 一行**:`_on_stat_changed` skip `source == PR_BREAKTHROUGH` — 否則 double-path(STAT_THRESHOLD provenance 搶先 + deferred flush crash window)。Approved-upstream additive amendment(EG-1 先例) | PR provenance + AC-13 flush 正確性 | #18 epic wiring story 內一行 #12 改動 + comment 修正 | #18 epic / #12 |
| **G-PR-6** | **#3 `pr.` namespace 註冊**:VALID_NAMESPACES(`persistence_layer.gd:291-294`)+ #3 GDD Rule 12 registry + `check_key_namespace_convention` lint 各一行 additive | debug build per-write push_warning | #18 epic story(additive;#3 L415 程序) | #18 epic / #3 |
...
> **Pass 1 review 處理紀錄**:16 BLOCKING clusters + Position A–F 裁決全部落本 revision(對照 `design/gdd/reviews/pr-detection-review-log.md` exit bar);ADR-0011 authored 同日。**Pending fresh re-review(full mode)— exit bar:0 new phantom,全 citation grep-verified。**
