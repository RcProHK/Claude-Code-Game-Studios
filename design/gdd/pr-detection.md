...
**Avatar Progression scope(ADR-0010 邊界)**:#26 AvatarRenderer **own** evolution tier 計算(由 #11 stat + #12 ability sync read derive)—— 本系統**唔重複**呢個邏輯。#18 嘅 progression 角色係**事件源**:PR → stat delta → #26 tier 自然推進;PR → ability unlock → #26 posture 變化。另加 PR milestone tracking(`pr_milestone_reached` — **#19 Zone System 已接做 unlock kind 之一**,zone-system.md L14/L39/L43/L64;thresholds PROVISIONAL 待 Q-PR-1 校準)。
...
| **#19 Zone System** | #19 own unlock framework | — | `pr_milestone_reached(count)` + `lifetime_pr_score` data surface | **#19 已接做 unlock kind**(zone-system.md L14/L39/L43/L64);CD directive:MVP zone 唔用,v0.2 + Q-PR-1 校準後 |
