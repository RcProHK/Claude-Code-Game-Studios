...
2. **Training-day count(unlock currency)** — 訂 #9 `workout_completed_forwarded(completed_at: int, transition_id: String)`(**shipped 現貨**:`workout_state_tracker.gd:69`;#9 係 fabrication-guarded validated path — count 永不行 raw #2,#20 教訓同款)。每事件:
   ...
   2. `utc_date(completed_at) <= last_counted_date` → no-op(**monotone date guard** — 兼任 per-calendar-day cap 1 同 stale-replay 防線:count 語意係「training-day 數」,同日 N session 只計 1,junk farming 物理免疫;**stale date(< cursor)同樣 no-op** — #2 epoch mismatch full-resync(ADR-0002 L60)會全史重派 event,monotone guard 令 #19 對 redelivery ordering **零假設**,error 方向永遠 under-count,anti-fabrication-safe)。Date 用 **UTC date derive 自 completed_at**(**注意單位:`completed_at` 係 unix ms** — `workout_state_tracker.gd:68`;`Time.get_datetime_string_from_unix_time()` 食 **seconds**,derive 前 ÷1000,否則垃圾 date 令 cap 永不觸發 = 防線靜默消失;純函數 deterministic、零 clock/timezone seam;同 #17 receipt date UTC determinism pin 同款。Local-day/UTC-day straddle 可移動 credit 歸屬日,但 total 受 distinct-UTC-date bound — 長期 rate 永遠 ≤1/day)。
...
4. **永久性(anti-pillar binding)** — `unlocked_zone_ids` **只加不減**(runtime mutation guard:唯一寫路徑係 Rule 3 append;load path validate + 異常 → EC-1 telemetry — **唔用 raw assert**,#18 AC-20 教訓);**ALWAYS zones derived-not-persisted**(manifest 只載 earned unlocks;可用集合 = derived ∪ persisted)。任何狀態變化(count 唔會跌,#11 同款 monotonic)都永不收回 zone。
5. **Persist + write-success-then-emit** — 單一 envelope key **`zone.state`**(SerializableResource,ADR-0006 C3 / ADR-0009;經 `to_dict()/from_dict()` — GSM tombstone / loot 先例):`{ schema_version: int, workout_count: int, last_counted_transition_id: String, last_counted_date: String, unlocked_zone_ids: Array[StringName], ceremony_pending: Array[StringName] }`。單 key 理由:count + 兩個 dedup cursor 係 **atomic pair**(分 key 寫,crash 喺中間 = double-count / lost count);#3 IPersistence 冇 enumeration API,whole-cache flush 下 per-key 零著數(#18 Rule 8 同款論證)。**順序 binding(#8 Rule 7 milestone-unlock 樣板 — `streak-system.md:285-293`:write 回 true 先 emit,false → erase rollback + halt;general atomic 紀律見 #8 Rule 3/9)**:`write("zone.state", envelope.to_dict(), flush=true)` 回 true **先** emit `zone_unlocked`;回 false → **唔 emit + in-memory rollback(範圍 = Rule 3 嘅兩個 append:`unlocked_zone_ids` + `ceremony_pending` 全部還原;`workout_count` / cursors **唔** rollback — deliberate:in-memory 保留,下次成功 persist 帶埋,boot reload 舊值亦 self-consistent)**+ `zone.persist_failed` telemetry — 下次 boot sweep(Rule 7)自愈(count 已 persist 嘅話 unlock 重新評估)。Emit 咗但冇 persist = ceremony 放咗領土消失 = anti-pillar,絕對唔准。
6. **Ceremony(P6 裁決)** — unlock 嘅 persist 效果**永遠即時**;presentation 分離:`zone_unlocked` emit 後,ceremony 交 `ceremony_pending` queue(persist 喺 envelope — survive crash)。**Delivery window**:唔遲過同 session 嘅 post-workout summary;mid-workout 觸發 → 經 #33 attention 通道排程(EC-5);boot-time sweep unlock(#20 scene-tree 未 wire,emit fire-into-void)→ queue 等 presentation ready 先 drain。**Multi-unlock aggregate**:queue drain 一次過做**單一 aggregated reveal**(「3 個 zone 迷霧散開」一個 ceremony),唔係 N 個 stacked toast。Drain API:`drain_ceremony_queue() -> Array[StringName]`(consumer 攞走即清;**drain 後 persist** — write 回 false → 唔 rollback drain(ceremony 已交付);殘留 queue 下次 boot 可能重播一次 aggregated reveal,accepted 方向(over-deliver ceremony,unlock state 不受影響)+ telemetry。Forward contract → #20/#29,BLOCKED-ON 標記見 Interactions)。
...
on workout_completed_forwarded(completed_at, transition_id):
    ...
    var day := utc_date_string(completed_at / 1000)                 # completed_at 係 unix MS — ÷1000 先入 seconds API
    if day <= state.last_counted_date: return                       # monotone guard:per-day cap + stale-replay no-op
...
*Variance:全部 input 嚟自 payload(server timestamp + transition_id)— deterministic,零 clock seam。同 UTC 日內第二個 workout 唔計(under-count 方向,安全)。**Monotone date guard 令 #19 對 redelivery ordering 零假設**:#2 epoch-mismatch full-resync(ADR-0002 L60 明文路徑)全史重派 → 全部 stale date ≤ cursor → 全 no-op;正常新 event(day > cursor)照 count。ISO "YYYY-MM-DD" 字串比較 = 字典序 = 日期序,`<=` 直接成立。*
...
- **EC-5 (LOW) — Mid-workout 達 threshold**:unlock persist 即時;ceremony 入 queue 經 #33 排程(window:唔遲過 post-workout summary);active zone 唔切(Rule 9)。**現行軸下 unreachable**(WORKOUT_COUNT 只喺 workout completion / boot sweep 觸發 — 兩者都唔係 mid-workout);defensive spec 留低,v0.2 PR_SCORE kind 先 activate。
- **EC-6 (LOW) — Duplicate zone_id / threshold < 1 / kind == UNKNOWN entry**:`validate_registry() == false` + push_error per entry(同 EC-4 path)。
- **EC-7 (MEDIUM) — Unlock persist write 回 false**:唔 emit + in-memory rollback(**`unlocked_zone_ids` + `ceremony_pending` 兩個 append 全還原**;count/cursors keep — Rule 5 範圍)+ `zone.persist_failed` telemetry;下次 boot sweep 自愈(ceremony 冇放、queue 冇 stale entry、領土冇消失 — 三面一致;sweep re-unlock 時 Rule 3 重新 append,零 duplicate)。
...
- **AC-02 (Logic)** — **GIVEN** 注入 registry 加第二 zone(WORKOUT_COUNT 2)+ count 1,**WHEN** `workout_completed_forwarded(t2, "txn-B")`(新 UTC day),**THEN** count → 2、`write("zone.state", _, flush=true)` **先於** `zone_unlocked` emit(shared-log spy order assert)、emit 一次、ceremony_pending 含該 zone_id、`zone.unlocked` telemetry emitted。
- **AC-03 (Logic)** — **GIVEN** AC-02 後,**WHEN** 同一 `"txn-B"` replay,**THEN** count 不變、零第二次 emit、零 queue 重複(EC-2)。
- **AC-04 (Logic)** — **GIVEN** persist 回 garbage(`_persistence` mock;**三個 vector**:non-Dictionary / 缺 schema_version / unlocked_zone_ids 錯 type),**WHEN** boot,**THEN** 每個 vector 都 ALWAYS zones 可用 + `zone.manifest_corrupt` telemetry(spy assert)、唔 crash(EC-1)。
...
- **AC-06 (Logic)** — **GIVEN** count 4 + threshold 5,**WHEN** `workout_completed_forwarded(t_same_day, "txn-C")`(`utc_date == last_counted_date`),**THEN** count 不變(per-day cap);**WHEN** `workout_completed_forwarded(t_stale, "txn-E")`(`utc_date < last_counted_date` — epoch full-resync 重派 scenario),**THEN** count 不變(monotone guard);**WHEN** `workout_completed_forwarded(t_next_day, "txn-D")`,**THEN** count → 5 + unlock(Rule 2)。Timestamps 用 **unix ms** fixture(UTC 邊界 pair:23:59:59Z vs 翌日 00:00:00Z pin `utc_date` derive 本身)。
...
- **AC-08 (Integration)** — **GIVEN** unlock + count + cursors + ceremony_pending persist,**WHEN** round-trip **必須經 flush 落盤 + fresh load(經 `JSON.parse_string` 路徑)— 唔可以同 instance cache read**(#3 `read()` 係 in-memory cache,同 instance write→read 零 JSON coercion = phantom pass),**THEN** envelope 完全還原,**特別 assert typed `Array[StringName]` fields(unlocked_zone_ids / ceremony_pending)正確 rebuild**(JSON 落地變 generic Array[String],`from_dict` 必須 rebuild/`assign()` — codebase 未有 typed-array round-trip 先例,呢度係第一個)(`tests/integration/zone_system/test_zone_persistence_roundtrip.gd`)。
...
- **AC-10 (Logic)** — **GIVEN** `write` mock 回 false,**WHEN** unlock 觸發,**THEN** 零 emit、`unlocked_zone_ids` rollback、**`ceremony_pending` 唔含該 zone_id(queue append 同步 rollback — EC-7 範圍)**、`zone.persist_failed` telemetry;**WHEN** 下次 boot(write 恢復),**THEN** sweep 補返 unlock + queue 單一 entry(零 duplicate)。
...
- **AC-12 (Logic)** — Boot lifecycle(functional form):**WHEN** `_ready` 完成,**THEN** state == READY(synchronous,唔跨 frame)+ #9 signal `is_connected` true + envelope 已 load(`_persistence` read spy ≥1);load≺sweep 由 AC-05(sweep 用到 loaded count)functional 證明 — sweep≺connect 喺 synchronous `_ready` 內 unobservable 兼 inconsequential,唔 assert。
