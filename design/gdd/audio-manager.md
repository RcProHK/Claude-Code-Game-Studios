...
- **Q-PENDING-BLK8-CO（Pass 4 BLOCKING resolved — needs #15 validation）**：LOOT_DROP from BOSS_ENCOUNTER → conditional boss_theme→rest_calm fade-back 已落入 Rule 6（see above）。依賴 #1 GSM `state_changed(from, to)` payload（confirmed present）+ **#15 LootDrop GDD 必須確認 boss kill → LOOT_DROP transition 嘅 from_state == BOSS_ENCOUNTER**（#15 GDD awaiting Pass 3 re-review，co-design marker）。Owner：#15 LootDrop GDD Pass 3 re-review。
